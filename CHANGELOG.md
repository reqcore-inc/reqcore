...
* **blog:** add Cluster 8 career page articles â€” pillar (career-page-that-converts) and two supporting articles (career-page-seo, google-for-jobs-structured-data)
...
* enhance LanguageSwitcher component with drop-up option and imprâ€¦ ([5aea684](https://github.com/WWMate-inc/WWMate/commit/5aea684d31412734786fd96b7816fc9322865936))
...
* **auth:** improve BETTER_AUTH_URL handling for Railway environmentsâ€¦ ([ef155aa](https://github.com/WWMate-inc/WWMate/commit/ef155aa8f35666f5ec129f2bb288365581a43138))
...
* **updates:** add API endpoints for update management, backup, changâ€¦ ([e8432e5](https://github.com/WWMate-inc/WWMate/commit/e8432e52cde8c035a704d26b2b7bd79523b3ce2e))
...
- **Blog: Best Free ATS Software for Startups (2026)** â€” Cluster 2 supporting roundup article. Compares 7 free ATS tools across three "free" models (free-forever, free trial, open source), with startup-specific evaluation criteria and upgrade signals. Published to `content/blog/best-free-ats-software-for-startups.md`
- **Internal links** â€” added cross-links from `best-open-source-applicant-tracking-systems.md` (Cluster 2 pillar) and `open-source-vs-free-ats.md` (Cluster 1) to new article

## 2026-03-07

### Added

- **Blog: Open Source vs Free ATS: Why They Aren't the Same** â€” Cluster 1 supporting article. Explains the difference between free (proprietary) and open source ATS, introduces a 4-category ATS pricing spectrum framework, and includes real infrastructure cost data. Published to `content/blog/open-source-vs-free-ats.md`
- **Internal link** â€” added cross-link from `open-source-applicant-tracking-system.md` (Cluster 1 pillar) to new supporting article

## 2026-03-04

### Added

- **Blog: How Does an Applicant Tracking System Work?** â€” Cluster 1 supporting article. Covers the ATS workflow from job posting to hiring decision, resume parsing mechanics, pipeline stages, candidate scoring methods (keyword vs rules vs AI), data ownership, and integrations. Published to `content/blog/how-applicant-tracking-systems-work.md`
- **Internal link** â€” added cross-link from `open-source-applicant-tracking-system.md` (Cluster 1 pillar) to new supporting article
...
- **Blog: OpenCATS vs WWMate: Open Source ATS Head-to-Head** â€” Published to `content/blog/opencats-vs-WWMate.md`
- **Internal link** â€” added cross-link from `best-open-source-applicant-tracking-systems.md` to new post

## 2026-02-22

### Added

- **Blog: Best Open Source Applicant Tracking Systems [2026]** â€” Cluster 2 pillar page. 3,800-word comparison of 7 open source ATS platforms with TCO analysis, evaluation framework, and FAQ. Published to `content/blog/best-open-source-applicant-tracking-systems.md`
- **Internal link** â€” added cross-link from `self-hosted-vs-cloud-ats.md` to new pillar page

### Fixed

- **Railway PR seed execution** â€” removed hard `.env` dependency from `db:seed`; seeding now works with platform-injected env vars and still supports local `.env` loading in `seed.ts`

### Changed

- **Unified Railway seeding path** â€” Railway predeploy now runs `db:seed` (same script as standard demo data), removing PR-specific seed divergence between preview and production-like environments
- **Preview demo defaults aligned** â€” runtime preview fallbacks now target `WWMate-demo` and `demo@WWMate.com` to match `server/scripts/seed.ts`

### Removed

- **PR-only seed script** â€” removed `server/scripts/seed-pr.ts` and the `db:seed:pr` npm script
...
- **Dependency security remediation** â€” resolved all `npm audit --audit-level=high` findings by upgrading `@aws-sdk/client-s3` (pulling patched `@aws-sdk/xml-builder`) and regenerating lockfile resolution
- **Transitive vulnerability pinning** â€” added npm `overrides` for `fast-xml-parser`, `minimatch`, `tar`, and `readdir-glob` to keep vulnerable transitive ranges out of the install graph
- **Demo write-protection enforcement** â€” hardened server demo guard so `POST`/`PATCH`/`PUT`/`DELETE` requests are consistently blocked for the configured demo organization and no longer silently fail open when demo org lookup fails
- **Dashboard preview UX** â€” write attempts in preview mode now trigger a dedicated upsell modal instead of only inline/API errors, while keeping action buttons visible

### Changed

- **Lockfile hygiene** â€” refreshed dependency graph with `npm install` + `npm dedupe` to remove stale vulnerable transitive entries
- **Demo env guidance** â€” `.env.example` demo slug example now matches seeded demo organization slug (`WWMate-demo`) to reduce configuration drift
...
- **Deployment platform migration** â€” migrated from Hetzner VPS (Caddy + systemd) to Railway (managed Nuxt service, Railway PostgreSQL, Railway Storage Buckets)
- **S3 path style now configurable** â€” `S3_FORCE_PATH_STYLE` env var controls path-style vs virtual-hosted-style S3 URLs (MinIO vs Railway Buckets/AWS S3)
- **S3 bucket plugin** â€” skips bucket initialization on managed providers (Railway/AWS) where buckets are pre-provisioned
- **`.env.example`** â€” expanded with full documentation, Railway-specific variable references, and all env vars

### Added

- **`start` script in `package.json`** â€” `node .output/server/index.mjs` for Railway Nixpacks detection
...
- **Organic SEO foundation** â€” `@nuxtjs/seo` (Sitemap, Robots, Schema.org, SEO Utils, Site Config) and `@nuxt/content` v3 (Markdown blog engine with typed collections)
- **Dynamic sitemap** â€” all open job postings auto-included via `/api/__sitemap__/urls`
- **Robots directives** â€” `/dashboard/`, `/auth/`, `/api/`, `/onboarding/` blocked from crawling; `noindex` on auth, onboarding, apply, and confirmation pages
- **JSON-LD structured data** â€” `JobPosting` on job detail (salary, location, remote, employment type), `Organization` + `WebSite` + `WebPage` on landing page, `Article` on blog posts
- **Job SEO fields** â€” `salaryMin`, `salaryMax`, `salaryCurrency`, `salaryUnit`, `remoteStatus`, `validThrough` added to job schema and all CRUD endpoints
- **Full OG + Twitter Card meta** on all public pages (landing, job board, job detail, roadmap, blog)
- **Blog** â€” listing page, article detail page with `@tailwindcss/typography` prose styling, seed article "Self-Hosted vs Cloud ATS: Pros, Cons, and When to Switch"
- **ISR route rules** â€” `/jobs/**` (3600s stale-while-revalidate), prerender for `/`, `/roadmap`, `/blog/**`
- **SVG favicon** â€” purple rounded rect with white "A"

### Changed

- **Landing page H1** â€” from "The recruitment engine you actually own" to "The open-source ATS you actually own" for keyword targeting
- **Landing page meta description** â€” optimized for "open source ATS", "self-hosted", "applicant tracking system" keywords
- **Public job API** â€” now joins organization table to expose `organizationName` for JSON-LD `hiringOrganization`
- **Navigation** â€” "Blog" link added to landing page navbar/footer and roadmap page navbar
...
- **In-app feedback** â€” floating button in the dashboard opens a modal to submit bug reports or feature requests directly as GitHub Issues. Server-side GitHub API integration with fine-grained PAT (token never exposed to client). Per-user rate limiting (5/hour). Auto-labels issues (`bug` / `enhancement`). Includes reporter context (name, email, page URL). Gracefully hidden when `GITHUB_FEEDBACK_TOKEN` / `GITHUB_FEEDBACK_REPO` env vars are not set.
- **Production deployment** â€” deployed to Hetzner Cloud CX23 (2 vCPU, 4GB RAM, Ubuntu 24.04) with Caddy reverse proxy, systemd service management, and one-command deploy script (`~/deploy.sh`)
- **Cloudflare CDN** â€” DNS, DDoS protection, edge caching, SSL termination (Full strict mode), and AI training bot blocking via Cloudflare Free plan
- **Deploy workflow** â€” `ssh deploy@server '~/deploy.sh'` pulls latest code, installs deps, builds, and restarts the systemd service
- **UFW firewall** â€” only ports 22 (SSH), 80 (HTTP), 443 (HTTPS) open

### Fixed

- **S3 bucket policy MinIO compatibility** â€” replaced `PutBucketPolicy` with `DeleteBucketPolicy` because MinIO doesn't support the `aws:PrincipalType` condition key used in the deny-anonymous policy; buckets without a policy are private by default in MinIO

### Changed

- **S3 bucket privacy strategy** â€” instead of setting an explicit deny-all policy (which used AWS-only condition keys), the startup plugin now deletes any existing bucket policy to ensure MinIO's default private behavior
...
- **Recruiter dashboard** (`app/pages/dashboard/index.vue`) â€” at-a-glance overview with four stat cards (Open Jobs, Total Candidates, Applications, Unreviewed), pipeline breakdown bar chart with color-coded status segments, jobs by status breakdown, recent applications list with relative timestamps, and top active jobs with new-application badges
- **Dashboard stats API** (`server/api/dashboard/stats.get.ts`) â€” single endpoint returning all dashboard data: summary counts, pipeline breakdown, jobs by status, recent 10 applications with candidate/job info, and top 5 active jobs by application count â€” all org-scoped with parallel query execution
- **Dashboard composable** (`app/composables/useDashboard.ts`) â€” wraps stats endpoint with computed unwrappers for all dashboard sections
...
- **Public roadmap page** (`app/pages/roadmap.vue`) â€” cinematic horizontal-scrolling timeline with 15 glassmorphism milestone cards, color-coded by status (shipped/building/vision), scroll-tracking progress glow on the timeline axis, smooth mousewheel-to-horizontal scroll conversion via requestAnimationFrame, and intro card centered on page load
- **Roadmap showcase section on landing page** â€” "Built in the open" section with mini timeline showing Shipped/Building/Vision counts and prominent CTA to full roadmap
- **Roadmap navigation links** â€” Roadmap link added to landing page navbar and footer
...
- **Dynamic sidebar job tabs** â€” when viewing a specific job (`/dashboard/jobs/:id/*`), the sidebar shows contextual sub-navigation: Overview, Pipeline, Application Form
- **Application Form tab page** (`app/pages/dashboard/jobs/[id]/application-form.vue`) â€” dedicated page for custom questions management and shareable application link
- **Sidebar icons** â€” all main nav items now display Lucide icons (LayoutDashboard, Briefcase, Users, Inbox, LogOut)
- **"All Jobs" sidebar back-link** â€” quick return to jobs list from any job sub-page

### Changed

- **Sidebar redesign** â€” replaced scoped CSS with Tailwind utility classes; added dynamic job context section with tab-based navigation
- **Dashboard layout** â€” removed `max-w-4xl` wrapper from `dashboard.vue`; each page now controls its own `mx-auto max-w-*` for proper centering
- **All dashboard pages** â€” added `mx-auto` to root elements for centered content within the main area
- **Dashboard index** â€” converted from `<style scoped>` to Tailwind utility classes
- **Job detail page** â€” removed "Back to Jobs" link, "View Pipeline" button, application link section, and Application Form Questions section (all moved to sidebar tabs / dedicated application-form page)
- **Pipeline page** â€” removed "Back to Job" link (sidebar provides navigation)

### Removed

- **"Back to X" links** on job sub-pages â€” sidebar now provides all navigation context
- **Scoped CSS** in `AppSidebar.vue` and `dashboard/index.vue` â€” replaced with Tailwind utilities
